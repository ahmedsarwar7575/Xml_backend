...
router.get('/', (req, res) => {
  try {
    ...
    res.json({ webshare_api_key, capsolver_key, captcha_enabled, headless_mode });
  } catch (err) {
...
router.put('/', (req, res) => {
  try {
    const { webshare_api_key, capsolver_key, captcha_enabled, headless_mode } = req.body;
    const stmt = db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)");
    if (webshare_api_key !== undefined) stmt.run('webshare_api_key', webshare_api_key);
    if (capsolver_key !== undefined) stmt.run('capsolver_key', capsolver_key);
    if (captcha_enabled !== undefined) stmt.run('captcha_enabled', captcha_enabled ? 'true' : 'false');
    if (headless_mode !== undefined) stmt.run('headless_mode', headless_mode ? 'true' : 'false');
